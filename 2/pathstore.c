...
static int SameFileIsInStore(Pathstore *store, char *pathname, char *retornoChksum );
static int IsSameFile(Pathstore *store, char *pathname1, /*char *pathname2*/ char *retornoChksum, PathstoreElement *e );
...
char *Pathstore_path(Pathstore *store, char *pathname, int discardDuplicateFiles) {
	...
	char retornoChksum[CHKSUMFILE_SIZE] ;

  numstores++;
  if (discardDuplicateFiles) {
    if (SameFileIsInStore(store,pathname, retornoChksum )) {
      numdups++;
      return NULL;
    }
  }
  ...
  //adicionado
  memcpy( e->chksum, retornoChksum, CHKSUMFILE_SIZE ) ;
 ...
 */
static int SameFileIsInStore(Pathstore *store, char *pathname, char *retornoCheckSum ) {
  PathstoreElement *e = store->elementList;

  chksumfile_bypathname( store->fshandle, pathname, retornoCheckSum );

  while (e) {
    if (IsSameFile(store, pathname, retornoCheckSum, e)) {
 ...
 */
static int IsSameFile(Pathstore *store, char *pathname1, /*char *pathname2*/ char *checkSum, PathstoreElement *e) {
  ...
  if (chksumfile_compare(checkSum, e->chksum) == 0) {
    numdiffchecksum++;
