...
HTTPCache::HTTPCache(unsigned short *portNumber, std::string **proxyServer, unsigned short *proxyServerPort)
{
...
void HTTPCache::atualizarStringDiretorio()
{
	if (*proxyServer != NULL)
	{
		cacheDirectory.append(to_string(*portNumber)) ;
	}
...
void HTTPCache::lockMutex(const HTTPRequest& request)
{
	hash<string> hasher ;
	size_t valorHash = hasher(serializeRequest(request)) ;

	pthread_mutex_lock(&mutexes[(valorHash % 997)]) ;
}


void HTTPCache::unlockMutex(const HTTPRequest& request)
{
	hash<string> hasher ;
	size_t valorHash = hasher(serializeRequest(request)) ;

	pthread_mutex_unlock(&mutexes[(valorHash % 997)]) ;
}
...
bool HTTPCache::containsCacheEntry(const HTTPRequest& request, HTTPResponse& response) const {
  ...
  try {
    ...
    cout << "     [Using cached copy of previous request for " << request.getURL() << ".]" << endl ;
    ...
    cerr <<"     [Problem rehydrating previously cached response for " << request.getURL() << ".]" << endl;
    cerr << "     [Operating as if cached entry doesn't exist and forwarding request to origin server.]" << endl ;
...
void HTTPCache::cacheEntry(const HTTPRequest& request, const HTTPResponse& response) {
  ...
  cout <<"     [Okay to cache response, so caching response under hash of "
       << requestHash << " for next " << response.getTTL() << " seconds.]" << endl ;

  PrintLock::unlock() ;

  ensureDirectoryExists(cacheDirectory + "/" + requestHash, true);
...
static const int kDefaultPermissions = 0777;
void HTTPCache::ensureDirectoryExists(const string& directory, bool empty) const {

	/*
	 * alterações nessa funçao pois essa versão nao funcionaria com multithreading. tambem possuia
	 * erros ao criar o diretório pela primeira vez
	 */
