...
HTTPProxy::HTTPProxy(int argc, char *argv[]) throw (HTTPProxyException) :
  ...
  scheduler(&portNumber, &proxyServer, &proxyServerPort)
{
  try
  {
    ...
    scheduler.getHTTPRequestHandler().cache.atualizarStringDiretorio() ;

    for (int i=0; i<997; i++)
    {
    	pthread_mutex_init( &mutexesSockets[i], NULL ) ;
    }
...
void HTTPProxy::acceptAndProxyRequest() throw(HTTPProxyException) {
  ...
  try {
    scheduler.scheduleRequest( connectionfd, clientIPAddress, mutexesSockets+(connectionfd%997) );
  } catch (...) {
...
void HTTPProxy::configureFromArgumentList(int argc, char *argv[]) throw (HTTPProxyException) {
  struct option options[] = {
	...
	{"proxy-port", required_argument, NULL, 'x'},
    {NULL, 0, NULL, 0},
  };
  ...
  while (true) {
    int ch = getopt_long(argc, argv, "p:s:x:nc", options, NULL);
    if (ch == -1) break;
    switch (ch) {
    ...
    case 's':
	  proxyServer = new std::string(optarg);
	  break;

    case 'x':
	  proxyServerPort = extractPortNumber(optarg);
	  break;
