...
void HTTPRequest::ingestRequestLine(istream& instream) throw (HTTPBadRequestException) {
  ...
  if (instream.fail()) {
	  ...
	  if ( instream.bad() )
		  throw HTTPBadRequestException( "Badbit setado" ) ;

	  if ( instream.eof() )
		  throw HTTPBadRequestException( "Eof setado" ) ;
...
void HTTPRequest::adicionarHeaderEntity( const std::string& name, const std::string& value ) {
...
const std::string& HTTPRequest::getValorHeaderComoString( const std::string& name ) {
...
void HTTPRequest::escreverRequestCompleto( std::ostream& os ) {

	os << method << " " << url << " " << protocol << "\r\n";
	os << requestHeader;
